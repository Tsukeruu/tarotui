...
main() {
  ...
  if [[ "$ollamaPrompt" = "Y" || "$ollamaPrompt" = "y" ]]; then 
    ...
    read -p "After creating the model: tarotui, would you like to delete the base model 'deepseek-r1', this would not affect anything in regards to the program (Y/N) " clean
